...
async function restoreSession() {
  try {
    const result = await request(`${API_BASE}/me`);
...
async function onLoginSubmit(event) {
  ...
  try {
    const result = await request(`${API_BASE}/login`, {
      method: "POST",
      auth: false,
...
async function loadDispatch() {
  try {
    ...
    const result = await request(`${API_BASE}/dispatch?${params.toString()}`);
...
async function loadFollowups() {
  try {
    ...
    const result = await request(`${API_BASE}/followups?${params.toString()}`);
...
async function onChangePasswordSubmit(event) {
  ...
  try {
    const result = await request(`${API_BASE}/password/change`, {
      method: "POST",
...
async function onResetPasswordSubmit(event) {
  ...
  try {
    const result = await request(`${API_BASE}/users/reset-password`, {
      method: "POST",
...
async function markFollowupDone(orderId, type) {
  ...
  try {
    await request(`${API_BASE}/followups/mark-done`, {
      method: "POST",
